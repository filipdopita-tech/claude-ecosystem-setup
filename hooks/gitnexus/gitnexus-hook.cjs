...
function resolveCliPath() {
  let cliPath = "/Users/filipdopita/.npm/_npx/e46929201c1128dd/node_modules/gitnexus/dist/cli/index.js";
...
function handlePostToolUse(input) {
  ...
  // Memory guard: skip reindex suggestion if system is under pressure
  try {
    const memOut = require('child_process').execSync('sysctl -n vm.swapusage', { encoding: 'utf-8', timeout: 2000 });
    const usedMatch = memOut.match(/used\s*=\s*([\d.]+)M/);
    if (usedMatch && parseFloat(usedMatch[1]) > 2048) {
      // System swap > 2GB — don't suggest heavy analyze
      return;
    }
  } catch { /* if sysctl fails, proceed normally */ }

  const analyzeCmd = `NODE_OPTIONS="--max-old-space-size=1024" npx gitnexus analyze${hadEmbeddings ? ' --embeddings' : ''}`;
  sendHookResponse('PostToolUse',
    `GitNexus index is stale (last indexed: ${lastCommit ? lastCommit.slice(0, 7) : 'never'}). ` +
    `Run \`${analyzeCmd}\` to update the knowledge graph (heap capped at 1GB for 8GB Mac).`
  );
}
