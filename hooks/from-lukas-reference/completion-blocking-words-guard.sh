#!/bin/bash
# completion-blocking-words-guard.sh — PreToolUse on Write/Edit
# Detects sycophantic / completion-blocking phrases in file content.
# Adapted from filipdopita-tech/claude-ecosystem-setup, generalized for Lukáš stack.
#
# Severity model:
#   Tier 1 single hit  → advisory (system-reminder inject)
#   Tier 1 3+ hits AND content > 500 chars (final-response material) → BLOCK (exit 2)
#   Tier 2 scope reduction → warning
#   Tier 3 deferral → warning
#
# Override: COMPLETION_OVERRIDE=1 env var bypasses block (logged).
# Cost: zero. Pure bash + jq + grep, no API calls.

LOG_FILE="$HOME/.claude/logs/completion-blocking-hook.log"
VIOLATIONS_LOG="$HOME/.claude/logs/completion-violations.jsonl"
...
# Skip meta files (legitimate anti-pattern discussion)
case "$FILE_PATH" in
  *"/.claude/rules/"*|*"/.claude/hooks/"*|*"/.claude/skills/"*|*"/.claude/scripts/"*|*"/.claude/agents/"*|*"/.claude/commands/"*|*"/.claude/CLAUDE.md"*|*"CLAUDE.md"|*"/scripts/"*|*"/memory/"*|*"feedback_"*|*"completion-mandate"*|*"settings.json"*|*"/expertise/"*|*"/knowledge/"*|*"/tests/"*|*"/__tests__/"*|*".test.ts"|*".test.tsx"|*".test.js"|*".spec.ts"|*".spec.js"|*"_test.go"|*"_test.py"|*"test_"*.py|*"/lukasdlouhy-claude-ecosystem/"*)
...
# =============================================================================
# Pattern bank (CZ + SK + EN + paraphrase + hedge)
# =============================================================================
TIER1_HITS=""
TIER1_COUNT=0
...
check_phrase "rozhodněte (se|si) (mezi|kterou)|co (vy )?preferujete|co vy na to" "rozhodněte-vy"
...
ZAKÁZANÉ vzorce ve final outputu (CZ/SK/EN):
- "není možné / to nejde / not possible" → "X selhalo přes Y, použil jsem Z"
- "potřebuji vaše schválení / I need your approval" → rozhodni sám
...
Logged: ~/.claude/logs/completion-violations.jsonl
Override (when legitimate): COMPLETION_OVERRIDE=1
</system-reminder>
EOF
fi
