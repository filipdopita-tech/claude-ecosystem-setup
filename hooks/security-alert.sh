...
if [ -n "$BAD_PERMS" ]; then
    curl -sS -d "Bad .env perms: $BAD_PERMS" -H "Title: OneFlow Security Alert" "https://ntfy.oneflow.cz/Filip" >/dev/null 2>&1
fi
...
for k in $ACTUAL_KEYS; do
    if ! echo "$EXPECTED_KEYS" | grep -q "$k"; then
        curl -sS -d "Unknown SSH key: $k" -H "Title: OneFlow SSH Anomaly" "https://ntfy.oneflow.cz/Filip" >/dev/null 2>&1
    fi
done
...
if [ -f "$BASELINE_FILE" ]; then
    ...
    if [ "$EXPECTED_AGENTS_HASH" != "$BASELINE_HASH" ]; then
        ...
        curl -sS -d "LaunchAgents changed - run audit" -H "Title: OneFlow LaunchAgent Change" "https://ntfy.oneflow.cz/Filip" >/dev/null 2>&1
    fi
else
