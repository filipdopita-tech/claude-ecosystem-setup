...
## Context injection (automatic — Wave 2 2026-05-09)

Every handoff now auto-injects project context BEFORE the task description:
- **Git log** (last 15 commits) — Codex sees recent history
- **Git status** — uncommitted changes visible
- **Project tree** (depth 2, capped 60 lines) — structure at a glance
- **AGENTS.md** (first 80 lines) — project-specific instructions (highest priority)
- **README.md** (first 25 lines) as fallback if no AGENTS.md

Add `AGENTS.md` to any project to give Codex persistent project-specific instructions
(coding conventions, forbidden changes, verification commands, file layout).

Projects with AGENTS.md: dialdeck, jobs-cz-system, distressed-leads, oneflow-telegram-drive, ciad-team-app, malpra-redesign-2026-05-05.

## Parallel execution (Wave 2 2026-05-09)

Run multiple independent Codex tasks simultaneously:

```bash
# Via ofs alias
ofs parallel "jobs-cz-system|refactor scraper for multi-portal" "dialdeck|bump deps and run tests"
ofs px "project1|task1" "project2|task2"

# Direct
~/Desktop/Codex/ai-control-plane/scripts/codex-parallel.sh \
  "jobs-cz-system|refactor scraper/leads.py" \
  "dialdeck|fix TypeScript strict errors"

# From file
codex-parallel.sh --file tasks.txt   # one "PROJECT|TASK" per line, # = comment
codex-parallel.sh --dry-run "proj|task"  # validate specs without running
```

Options: `--max N` (default 4 concurrent), `--dry-run`, `--file FILE`.
Telemetry: parallel_run events appear in `ofs bu` summary.

## Output

Each run produces 3 files in `~/Desktop/Codex/ai-control-plane/handoffs/`:
- `<ts>-codex-<project>.md` — handoff (context snapshot + task + operating rules + report contract)
- `<ts>-codex-<project>.result.md` — Codex output
- `<ts>-codex-<project>.verify.md` — git diff snapshot + claim/diff mismatch flags

Plus telemetry: append to `~/.claude/logs/bridge-utilization.jsonl` (consumed by `weekly-retro.sh` + `ofs bu`).
