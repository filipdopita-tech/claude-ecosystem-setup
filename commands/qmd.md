...
## Reranking (precision mode)

After a standard QMD query, pipe the JSON output through the cross-encoder reranker for +20-35% precision. Use when result quality matters more than latency (DD research, source lookups, surfacing results to Filip).

```bash
# Full precision pipeline
qmd query --json "lex: DSCR emitent" | python3 /root/.claude/skills/qmd/rerank.py

# With explicit JSON input
echo '{"query":"OneFlow DSCR","results":[...],"top_k":5}' | python3 /root/.claude/skills/qmd/rerank.py
```

**Input format** (stdin JSON): `{ "query": "...", "results": [...qmd objects...], "top_k": 10 }`
**Output**: same objects sorted by `rerank_score` (0-1), top_k returned.
**First run**: downloads ~67MB model cross-encoder/ms-marco-MiniLM-L-6-v2 (Apache 2.0, cached in ~/.cache/huggingface/).
**When to use**: precision-critical lookups (DD research, complex multi-concept queries). Skip for latency-sensitive bulk ops.
