...
### Layer 0 — BM25 hybrid-recall (fast ranked search, ~150ms, 0 tokens)

```bash
python3 ~/scripts/automation/hybrid-recall.py "<term>" --top 5 --verbose
```

Searches all 482+ memory/*.md files via BM25 (Robertson k1=1.5, b=0.75). Returns ranked hits with scores and excerpts. Cache-backed — instant on repeat queries. **STOP** if score ≥4.0 and title matches topic. Read the returned file path.

Fallback: if `hybrid-recall.py` missing, skip to Layer 1.
