...
| `/prompt-master` `/prompt` | **Tool-aware** prompt engineer (Claude/Cursor/Midjourney/fal/Krea/Kie/seedance/HyperFrames/coding agents). Tool routing + hard rules vs hallucination patterns + reasoning-native model awareness. OneFlow brand context auto-injected. | **Default** pro jakýkoli prompt design — image/video/code/copy. Replaces /megaprompt, /promptfix, /autoprompt for tool-targeted output. |
| `/image-prompt` | Chain helper: prompt-master + image AI tool routing (fal/Krea/Kie/MJ) + OneFlow brand lock | Image gen tasks |
| `/megaprompt` | Generate detailed comprehensive prompt (tool-agnostic) | Když mám rough idea → potřebuju long spec, ne tool-targeted output |
| `/promptfix` | Fix vague/bad prompt | Quick refine bez tool routing |
| `/autoprompt` | Build perfect prompt from rough idea (recursive) | Recursive self-improvement |
