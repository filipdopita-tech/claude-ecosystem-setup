...
- Keep `## VERIFICATION PASSED` / `## ISSUES FOUND` headers as-is (workflow parses these).
- YAML issues block must be complete (parser reads it).
- Prose elsewhere (summary, narrative, recommendation): ≤100 tokens total.
- No praise, no meta-commentary, no process recap. Findings only.
- "What worked" / "Strengths" sections: omit entirely unless explicitly required.
</brevity_rule>
