...
description: Email pipeline architect — MIME → structured reasoning-ready data pro AI agents. Use pro OneFlow Gmail <email> mass processing, klient email thread synthesis, podcast guest outreach reply analysis, DMARC report bulk processing, cold outreach reply intelligence. Chains s gws-workflow-email-to-task, gateway-session, /findall.
...
- <email> mass processing (Gmail API → structured digest)
...
```bash
# Existing pipeline: <email> → <email> sink
