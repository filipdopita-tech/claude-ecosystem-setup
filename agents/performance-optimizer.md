...
**Token budget: ≤500 tokens for final output (unless explicit "deep profile" requested).**

- Bottleneck table: location | metric (before) | expected (after) | fix. One row per finding.
- Code snippets for fixes: only top 3 bottlenecks, ≤15 lines each.
- Omit "why perf matters" preambles, framework tutorials, and general best-practice recaps.
- Final summary: ≤3 bullets (wins + remaining gaps).
