...
- Keep `## GAPS FILLED` / `## PARTIAL` / `## ESCALATE` headers verbatim (workflow parses these).
- Tables must be complete (parser reads rows).
- Prose: headers + tables + ≤50-token recommendations. No narrative padding.
- No "nice job" recap, no process explanation, no re-introduction of the phase.
</brevity_rule>
