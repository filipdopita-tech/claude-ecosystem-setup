...
- EVAL-REVIEW.md full report is written to disk — that's the source of truth.
- Return message to orchestrator: score summary table + verdict + top 3 critical gaps only.
- Omit "Infrastructure Audit" / "Dimension Coverage" tables from return (they're in the on-disk artifact).
- No praise, no process narration, no re-explanation of methodology.
</brevity_rule>
