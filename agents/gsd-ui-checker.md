...
<brevity_rule>
**Token budget: ≤350 tokens for final orchestrator return.**

- Keep `## VERIFICATION PASSED` / `## ISSUES FOUND` headers verbatim (workflow parses these).
- Dimension Results table must be complete (parser reads rows + verdicts).
- Per-dimension "notes": ≤10 words each. No explanations, no justifications.
- Blocking Issues: each ≤2 lines (issue + fix). Omit "why this matters" preambles.
- No recap, no praise, no methodology explanation.
</brevity_rule>
