...
description: Onboarding nového klienta pro Meta Ads — 2 cesty (A=per-klient System User pro IHNED, B=OneFlow App Live mode pro permanent scaling). Replikuje proven <klient> pattern (2026-04-29). Trigger - "onboarduj klienta X na Meta Ads", "nový klient pro reklamy", "setup Meta Ads pro klienta", "klient mi dal admin v BM", "rozbal Meta Ads nabídku pro klienta".
allowed-tools:
  - Bash
  - Read
  - Write
...
**Současný status:** OneFlow App `1587515299033044` v Dev mode → App Review submission READY (`~/Documents/<klient>-project/APP-REVIEW-SUBMISSION-2026-04-29.md`). **Submit IHNED → Path B = default forward.**
...
3. Submission package: `~/Documents/<klient>-project/APP-REVIEW-SUBMISSION-2026-04-29.md` (use case + screencast script READY)
...
Reference template (modificuj per klient): `~/Documents/<klient>-project/create_12_leadgen_ads.py`

### Fáze 5: Filip → 1-click activate (10 sec)

```bash
# Filip stiskne 1× → kampaň jde live
CLIENT_OWN_TOKEN=$(cat /root/.credentials/<klient>_meta.env | cut -d= -f2)
curl -X POST "https://graph.facebook.com/v23.0/<CAMPAIGN_ID>" \
  -d "access_token=$CLIENT_OWN_TOKEN" -d "status=ACTIVE"
...
| Special ad category | FINANCIAL_PRODUCTS_SERVICES | Compliance pro CZ finanční klienty (<klient>, OneFlow) |
...
**Files (template):**
- `~/Documents/<klient>-project/create_12_leadgen_ads.py` — programmatic creator
- `~/Documents/<klient>-project/100-AUDIT-2026-04-29.sh` — 14-dim audit
- `~/Documents/<klient>-project/campaign-state.json` — state schema
