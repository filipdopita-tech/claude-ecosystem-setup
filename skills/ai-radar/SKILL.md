---
name: ai-radar
description: Skenuj AI ekosystém (Anthropic, OpenAI, GitHub trending, HN, Reddit, MCP registry, arxiv) za X dní, filtruj na [YOUR_COMPANY] stack, skóruj, auto-implementuj trivialitu, zbytek pošli do review-queue. Default = týdenní scan. Používá gh + curl + Gemini (batch >80K tokenů) pro 0 nákladů.
triggers:
  - ai radar
  - skenuj AI
  - co je nového v AI
...
# /ai-radar — AI ekosystém audit pro [YOUR_COMPANY]

## Účel

Rychlý filtrovaný přehled novinek ze světa AI/dev tools za posledních N dní (default 7), auditem proti [YOUR_COMPANY] stacku, auto-implementací nízkorizikové triviality a review-queue pro ostatní. **Není to generic research** (na to je `/last30days`). Tohle je **kurátor + auditor + implementátor** pro [YOUR_NAME] ekosystém.
...
- Když [YOUR_NAME] řekne: "co je nového v AI", "skenuj novinky", "ai radar"

## NIKDY

- Nespouštěj automaticky přes cron ([YOUR_NAME] musí spustit — review gate)
...
FÁZE 2: Filter        → [YOUR_COMPANY] relevance (Claude Code stack, VPS-PRIMARY, lead-gen, content)
...
Cost guard: `~/.claude/mcp-keys.env` má `GEMINI_API_KEY`. Pokud Gemini 429 → fallback OpenRouter `google/gemini-2.5-flash` (viz [your_name]-autopilot.md). NIKDY Vertex AI (cost-zero-tolerance).
...
## FÁZE 2 — Filter ([YOUR_COMPANY] relevance)

### Stack matrix (co [YOUR_NAME] už používá)
...
1. **HARD EXCLUDE** (skip bez auditu):
   - Pouze visual (graphic design tools ne[YOUR_COMPANY]-brand)
   - Gaming, crypto trading, meme coins
   - Vertical který [YOUR_NAME] nedělá (healthcare, edu, legal-tech pro US market)
...
3. **AUTO INCLUDE** (skip filter, vždy audituj):
   ...
   - MCP protocol update
   - Tool [YOUR_NAME] již watchlistuje (`reference_tool_watchlist.md`)
...
| Dimenze | Otázka | Váha |
|---|---|---|
| **Fit** | Jak dobře zapadá do [YOUR_COMPANY] stacku? (1=okrajově, 5=přímo nahrazuje/rozšiřuje existující komponentu) | ×3 |
...
- **Impact** (1-5): X — co to [YOUR_NAME] dá
- **Total skóre**: XX/45
...
- Systemové změny mimo ~/.claude/ a $VPS_MOUNT/scripts/
...
~~Nový skill~~ → **blokováno** (nové user-invocable commands vyžadují [YOUR_NAME]ovu review)

Pro cokoli jiného (včetně ~~cross-offs~~ nahoře) → **REVIEW_QUEUE** ([YOUR_NAME] schválí přes `/apply-improvements`).
...
- [ ] Položka 2: manuální eval [YOUR_NAME]em
- [ ] ...
```

### WATCHLIST

Položky 15-24 skóre → append do `~/.claude/ai-radar/watchlist.md` (primary; self-contained). Pokud existuje Obsidian vault a je writable, **zrcadli** i do `~/Documents/[YOUR_COMPANY]-Vault/02-Reference/ai-radar-watchlist.md`. **POVINNĚ** `mkdir -p` parent dir před Write operací (ověřeno 2026-04-21: [YOUR_COMPANY]-Vault/02-Reference neexistuje při první instalaci). Re-check za 30 dní.
...
## Výstup [YOUR_NAME] (stručně, max 200 slov úvod)

```markdown
# AI Radar — {date}, window {N} days

**Scan**: 8 zdrojů, {total} findings, {filtered} po [YOUR_COMPANY] filteru, {audited} auditováno.
...
~/.claude/review-queue/
└── ai-radar-YYYY-MM-DD.md    # pending [YOUR_NAME] approval

~/.claude/ai-radar/
└── watchlist.md              # 30-day re-check položky (primary, self-contained)

~/Documents/[YOUR_COMPANY]-Vault/02-Reference/    # optional mirror, vyžaduje mkdir -p před write
...
| `--skip-auto` | off | Vše pošli do review-queue (i HIGH skóre) — když [YOUR_NAME] chce review všechno |
...
13. Output [YOUR_NAME] (shrnutí + next steps)
...
[ ] Blast radius ≤ 2 soubory v ~/.claude/ nebo ~/Documents/[YOUR_COMPANY]-Vault/
...
- **`MEMORY.md`** — nová runs se loggují jako reference memory pokud [YOUR_NAME] schválí
...
- arxiv SKIP (signál-noise ratio nízký pro [YOUR_COMPANY] scope; re-add pokud [YOUR_NAME] požádá)
...
Interní [YOUR_COMPANY] skill. Zdroje: public RSS/API feeds, žádné scraping proti ToS.
