---
name: ai-radar
description: Unified AI & Ecosystem Radar v3. Skenuje (a) cizí AI ekosystém z 12 zdrojů (Anthropic CC+API+Cookbook, Plugin Marketplace, Awesome Lists, OpenRouter free, OAI, Google AI, GitHub trending, HN, Reddit, MCP registry), (b) tvůj vlastní Claude Code stack (8 dimenzí — services/evals/credentials/memory/skills/hooks/MCPs/knowledge-router) a (c) cross-reference engine s 5 kategoriemi (ALREADY_HAVE / DEPRECATED_PATTERN / COVERAGE_GAP / DECAY_SIGNAL / NEW_MCP_AVAILABLE). Mythos-grade audit (Bayesian falsification + ACH + calibrated confidence + source quality boost + learning loop). Default = oba paralelně + cross-ref + REAL auto-implement engine (6 safe action types). Auto-implementuje triviálně bezpečné změny do reálných souborů (memory references, watchlist, knowledge-router lines, hook chmod, watchlist prune), zbytek do review-queue pro /apply-improvements. 0 Kč (gh + curl + lokální bash; volitelně OpenRouter free pro batch).
triggers:
  - ai radar
  - ekosystem audit
  - skenuj ai
  ...
  - ecosystem scan
  - radar
argument-hint: "[--scope=external|internal|all] [--days=7] [--focus=claude-code|agents|scraping|cold-email|content|frontend|all] [--dry] [--no-cross-ref] [--full-effort] [--skip-auto] [--lite] [--max-auto=N]"
allowed-tools:
  - Bash
  - Read
  - Write
...
# /ai-radar — Unified AI & Ecosystem Radar v3

> **One radar. Two telescopes. One unified picture. Real implementation engine.**
> External (12 zdrojů) + Internal (8 dim) + Cross-reference engine (5 kategorií) + REAL auto-implement (6 action types). Mythos-grade reasoning. Zero cost.
...
## Architektura (5 vrstev, mythos-grade, v3 enhanced)

```
┌─────────────────────────────────────────────────────────────────────┐
│ VRSTVA 1 — DISCOVER (paralelní fetch)                                │
│   ├─ EXTERNAL: 12 zdrojů (v3: +Cookbook +Plugin MP +Awesome +ORouter)│
│   └─ INTERNAL: 8 dimenzí (services, evals, credentials, memory,      │
│                skills, hooks, MCPs, knowledge-router)                │
├─────────────────────────────────────────────────────────────────────┤
...
├─────────────────────────────────────────────────────────────────────┤
│ VRSTVA 5 — ROUTE + IMPLEMENT (REAL engine v3)                        │
...
8. **Vrstva 5 — Route + Implement**: `bash scripts/unified-router.sh` agreguje + volá `auto-implement.sh` pro AUTO actions
...
|---|---|---|---|---|
| `APPEND_TOOL_WATCHLIST` | append řádek do `memory/reference_tool_watchlist.md` | 30 | LIKELY | grep -F URL |
...
- Existing scanner contracts (scan-internal.sh, dim_*) zachovány
- Existing audit-engine.py output schema zachován (jen +source_boost, +learning_boost ve scores)
- Existing unified-router.sh interface zachován (router teď interně volá auto-implement.sh)
- Existing cron entries: legacy `~/.claude/ecosystem-radar/run-radar.sh` budou odstraněny během cron-consolidation kroku
- Nové scripts: `auto-implement.sh`, `prune-watchlist.sh` — drop-in
- Nové reference docs sections updated
...
git -C ~/.claude/projects/-Users-filipdopita-Desktop-Codex/memory/ checkout reference_tool_watchlist.md
```
...
| `--scope=all` (default, v3) | 60-150s | ~5000 |
...
**Verzování:** scripts/ obsahují F-XXX iteration tagy:
- F-001..F-099: v1 baseline
- F-100..F-119: v3 — nové zdroje (Cookbook, Plugin MP, Awesome, ORouter), source quality boost
- F-120..F-129: v3 — NEW_MCP_AVAILABLE cross-ref + auto-implement engine

**Author:** Filip Dopita / OneFlow ecosystem.
**Version:** v3.0 (real auto-implement engine) — 2026-05-03
**Replaces:** ai-radar v2.0 (unified scan only) — 2026-04-29
