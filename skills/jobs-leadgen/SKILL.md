---
name: jobs-leadgen
description: "Reverse-recruiter pipeline pro CZ job-boardy (Jobs.cz/Práce.cz/StartupJobs/Profesia). Scrape job postings → detect pain signál (repost frequency + urgency wording + role match) → enrich firma+kontakt (ARES + Hunter + Apollo) → AI generate personalized pitch (Claude + OneFlow brand voice) → outreach z <email> → reply tracking → push GHL CRM. Trigger: 'reverse-recruiter', 'jobs.cz scrape lead-gen', 'monetizovat hiring pain', 'firmy které hledají X', 'oslov firmy co inzerují roli', '/jobs-leadgen'. Source: research briefing 2026-05-04 (5 docs ~10800 slov)."
...
- Send infrastructure: `<email>` (recommended) vs `<email>` (NE pro mass)
...
- **`<email>` only** — nikdy z `<email>` (chrání primary identity reputation)
