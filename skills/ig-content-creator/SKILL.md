...
description: "Vytvoř IG obsah pro OneFlow: carousel, reel script, static post. 3 tóny: Patient Observer / Dramatic Prophet / Quiet Devastator. Hook -> tón -> struktura -> caption -> CTA."
compatibility: No external API needed. Reads brand assets from ~/Documents/oneflow-claude-project/.
metadata:
  allowed-hosts: []
  version: "2.0"
---

# /ig-content-creator — OneFlow IG Content Creator
...
1. `~/Documents/oneflow-claude-project/PROJECT_INSTRUCTIONS.md` — Brand DNA, hooks, anti-robotic, CTA
2. `~/.claude/rules/filip-style-clone.md` — Filip styl, banned words
3. `~/.claude/rules/oneflow-all.md` — OneFlow voice, visual, content rules
...
Pokud Filip nespecifikuje tón, vyber **1 nejlepší variantu** pro téma a na konci nabídni ostatní 2.
...
- **Relevance**: sedí na OneFlow brand a aktuální pilíř?
...
```
Pravidla: Mluvit jako Filip (přímý, žádné omluvy). Aktivní slovesa. Žádné "v dnešní době".
...
**Checklist:**
- [ ] Zní jako Filip? (přímý, sebevědomý, žádné omluvy)
...
Brand fit:     [1-10] — sedí na OneFlow voice + monochrome?
...
**Evolving Brain (po schválení Filipem):**
Jakmile Filip schválí post nebo sdílí výkon (saves, reach, komentáře), zaznamenej do Graphiti:
```
...
1. **Nepiš bez brand kontextu.** VŽDY nejdřív načti PROJECT_INSTRUCTIONS.md + oneflow-all.md.
2. **Nepoužívej banned words.** Viz oneflow-all.md. "Inovativní", "revoluční", "synergie" = okamžitý fail.
